...
# Read random
percentages_random <- read_csv("/home/agalvez/data/metabolic_amoeba/version4/data/general/random_data.csv") |> 
...
rows_to_remove1 <- tips_taxons |>
  filter(Supergroup %in% c("Outgroup","Ancestor-Bpp")) |>
  pull(RowName)
...
# Merge both tables
amorphea_COG_percentages <- rbind(percentages_per_species,percentages_ancestors,percentages_random) |> 
...
final_ca <- ggplot() + 
  ...
  theme_minimal() +
  labs(x = "Dimension 1 (51.2%)", y = "Dimension 2 (13%)"
  )

ggsave("/home/agalvez/data/metabolic_amoeba/version4/results/final_figures/pre_inkscape_random_ancestors_notung_ca.pdf", plot = final_ca, width = 10, height = 8)
