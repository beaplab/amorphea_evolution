...
rows_to_remove1 <- tips_taxons |> 
  filter(!(Supergroup %in% c("Metazoa", "Olympicamoeba", "Amoebozoa"))) |> 
  pull(RowName)
...
final_ca <- ggplot() + geom_polygon(data = hull_data, aes(x = Dim.1, y = Dim.2, fill = Supergroup, color = Supergroup, group = Supergroup),
  ...
  scale_fill_manual(values = c("Metazoa" = "red", "Amoebozoa" = "purple", "Olympicamoeba" = "orange")) +  # Customize fill for the convex hulls and color for points and labels
  scale_color_manual(values = c("Metazoa" = "red", "Amoebozoa" = "purple", "Olympicamoeba" = "orange")) +   # Customize point and label colors
  theme_minimal() +
  labs(x = "Dimension 1 (36.8%)",
       y = "Dimension 2 (15.6%)") 

ggsave("/home/agalvez/data/metabolic_amoeba/version4/results/final_figures/pre_inkscape_metazoa_vs_amoebozoa_ca.pdf", plot = final_ca, width = 10, height = 8)
